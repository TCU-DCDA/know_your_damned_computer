...
function initQuizEngine() {
    ...
    if (!window.quizEngine) {
        ...
        // If engine exists, just re-scan for new quizzes (e.g. after navigation)
        window.quizEngine.init();
    }
    
    window.quizEngineInitialized = true;
}
...
window.addEventListener('quarto:page-load', () => {
    window.quizEngineInitialized = false; // Reset flag for new page
    initQuizEngine();
});

// Add quiz-specific styles (uses Bootstrap CSS variables for light/dark compatibility)
const quizStyles = `
<style>
.quiz-question {
    margin-bottom: 1rem;
...
.quiz-option.correct {
    background: var(--bs-success-bg-subtle, rgba(5, 150, 105, 0.15));
    border-color: var(--bs-success, #059669);
    color: var(--bs-body-color);
}

.quiz-option.incorrect {
    background: var(--bs-danger-bg-subtle, rgba(220, 38, 38, 0.15));
...
.feedback.correct {
    background: var(--bs-success-bg-subtle, rgba(5, 150, 105, 0.15));
    border-left-color: var(--bs-success, #059669);
    color: var(--bs-body-color);
}
